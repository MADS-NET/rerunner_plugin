...
public:
  ...
  return_type load_data(json const &input, string topic = "",
    ...
    double time_seconds;
    // get the timecode from the input json if available, otherwise fall back
    // to wall-clock (Unix epoch) time, so independent processes logging to
    // the same "time" timeline (e.g. video.py) stay mutually synchronized
    // instead of each being relative to its own process start.
    if (!_params["time"].empty() && input.contains(_params["time"])) {
      time_seconds = input[_params["time"]].get<double>();
    } else {
      time_seconds = std::chrono::duration<double>(
    ...
    if (_params["parallelize"].get<bool>()) {
      ...
      for (const auto &keypath : tk.scalars) {
        ...
        if (value.is_number()) {
          ...
          _rec->log("states/" + keypath, rerun::TextLog(value.get<string>()));
          logged = true;
        }
      }
    ...
    for (const auto &keypath : tk.traces) {
      ...
      if (point.is_array() && point.size() == 3) {
        _rec->log("trace/" + keypath,
                  rerun::Points3D(rerun::Position3D(point)));
        logged = true;
      } else if (point.is_array() && point.size() == 2) {
        _rec->log("trace/" + keypath,
                  rerun::Points2D(rerun::Position2D(point)));
        logged = true;
      } else {
        _error = "Trace keypath " + keypath +
                 " does not point to a valid 2D or 3D point array";
        return return_type::error;
      }
    }
  ...
  void set_params(const json &params) override {
    ...
    if (_params.contains("recording_id") &&
        _params["recording_id"].is_string()) {
      _rec = std::make_shared<rerun::RecordingStream>(
          "MADS " + _params.value("agent_name", "rerunner (generic)"),
          _params["recording_id"].get<std::string>());
    } else {
      _rec = std::make_shared<rerun::RecordingStream>(
          "MADS " + _params.value("agent_name", "rerunner (generic)"));
    }
    ...
    if (!_params["skeleton"].get<string>().empty()) {
      ...
      _skeleton->show_axes(_params.value("show_axes", true),
                           _params.value("axes_length", 1000.0f));
  ...
  map<string, string> info() override {
    ...
    return {
        ...
        {"Scene file path", _params["scene_file_path"].get<string>().empty()
                                ...
                                : _params["scene_file_path"].get<string>()},
        {"Scene rotation:",
         std::to_string(_params.value("scene_file_y_rot", 0.0))},
        {"Time column", _params["time"].get<string>().empty()
                            ? "timecode"
                            : _params["time"].get<std::string>()},
        {"Blueprint", (_blueprint.empty() ? "None" : _blueprint.string())},
        {"Parallelize", _params["parallelize"].get<bool>()
                            ? to_string(_pool.get_thread_count()) + " threads"
                            : "NO"}};
  };
};
